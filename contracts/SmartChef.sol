...
// added functionnalities by the littlerabbit finance's devs
//
...
contract SmartChef is Ownable {
    ...
    // The block number when LTRBT mining ends.
    uint256 public bonusEndBlock;
    // Deposited amount LTRBT in SmartChef
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 lpSupply = depositedLtrbt;
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 ltrbtReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Stake LTRBT tokens to TheBushV1
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if(pool.depositFeeBP > 0){
                ...
                depositedLtrbt = depositedLtrbt.add(_amount).sub(depositFee);
            }else{
                user.amount = user.amount.add(_amount);
